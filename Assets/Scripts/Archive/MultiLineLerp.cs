...
public class MultiLineLerp : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (goodToGo)
        {
            ...
            if (distCovered < journeyLength)
            {
                ...
                newPointL = Vector3.Lerp(LlerpStart, LlerpEnd, fracJourney);
                newPointD = Vector3.Lerp(DlerpStart, DlerpEnd, fracJourney);
                newPointU = Vector3.Lerp(UlerpStart, UlerpEnd, fracJourney);

                //Generate the next point in the line
                Instantiate(_CurvePoint, newPoint, Quaternion.identity, transform);
                Instantiate(_CurvePoint, newPointR, Quaternion.identity, transform);
                Instantiate(_CurvePoint, newPointL, Quaternion.identity, transform);
                Instantiate(_CurvePoint, newPointD, Quaternion.identity, transform);
                Instantiate(_CurvePoint, newPointU, Quaternion.identity, transform);
    ...
    void ResetPointLerp()
    {
        ...
        if (firstRun)
        {
            ...
            LlerpEnd = pointsDown[counter];
            DlerpEnd = pointsUp[counter];
            UlerpEnd = pointsRight[counter];
        ...
        else
        {
            ...
            LlerpStart = pointsDown[counter - 1];
            DlerpStart = pointsUp[counter - 1];
            UlerpStart = pointsRight[counter - 1];
